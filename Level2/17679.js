function solution(m, n, board) {
  board = board.map((item, _) => item.split(""));
  while (true) {
    let founded = [];
    for (let k = 1; k < m; k++) {
      for (let l = 1; l < n; l++) {
    ...
    if (!founded.length) {
      return [].concat(...board).filter((item) => !item).length;
    }
...
// 다른사람의 풀이 - 제네레이터 이용
// const unique = list => list.filter((ar0, idx, list) => list.findIndex(ar1 => ar0[0] === ar1[0] && ar0[1] === ar1[1]) === idx);
...
// const rotateList = (ar, n) => createOneValueList(null)(n).map((e, i) => ar.map(b => b[i]));
...
//     if(allPass(!(equalToBase(EMPTY)), equalToBase(rightBlock), equalToBase(bottomBlock), equalToBase(diagonalBlock)))
//         yield [ getBaseIndex(fIdx, sIdx), getRightIndex(fIdx, sIdx), getBottomIndex(fIdx, sIdx), getDiagonalIndex(fIdx, sIdx) ];
// }
...
// }
// function solution(m, n, board) {
//     const blocks = rotateList(board.map(e => e.split("")), board[0].length);
